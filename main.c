...
/* files from /etc/init might be vulnerable as well */
char *start_files[] = {"/etc/modules", "/etc/rc.local", "/etc/inittab",
	                  "/etc/rc.d/rc.sysinit", "/etc/init.d/rc" };
int i;    // `shared' counter variable


/* Tries to read the hidden content of the vulnerable file. *
 * Uses mmap() syscall                                      */
...
/* reads the file in portions, seeking each time. *
 * uses fread()                                   */
int portions_fread(FILE *f, char *buf, unsigned int buf_size)
{
	int i, b_read=0;
	char c[2] = {0,0};
	rewind(f);
	printf("Going to read %i bytes %i times\n", 1, buf_size );
	for(i = 0; i<buf_size; i+=1){
		
		/* if the rootkit is `smart' and starts hiding with one single *
		 * char, we should retry reading, starting 1 byte before. This *
		 * may fail when hidden content is placed in the beginning of  *
		 * file (quite unlikely for now)                               */
		if (fread((c+1), 1, 1, f)){
			if (feof(f))
			    break;
			fseek(f, -2, SEEK_CUR);
			...
			b_read+=1;
		}  //b_read += fread(&c, 1, 1, f);
		sprintf(buf+strlen(buf), "%c", *(c+1));
		fseek(f, i+1, SEEK_SET); // without this line the rootkit makes its deal
	}
    buf[i-1] = 0;
	if(ferror(f)){
